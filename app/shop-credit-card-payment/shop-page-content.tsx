        ...
        <main className="min-h-screen bg-white">
            ...
            <header className="border-b border-gray-100 bg-linear-to-b from-gray-50/80 to-white">
                <div className="mx-auto max-w-7xl px-6 pb-10 pt-28 lg:px-8 lg:pb-12 lg:pt-36">
                    ...
                    <h1 className="mt-3 font-serif text-4xl tracking-tight text-deep-space-blue lg:text-6xl">ซื้อสินค้า IMAGE AUTOMAT และชำระด้วยบัตรเครดิต</h1>
...
function ProductCard({ product: p, isAuthed }: { product: ShopProduct; isAuthed: boolean }) {
    ...
    return (
        <Link
            href={`/shop-credit-card-payment/${p.id}`}
        ...
        >
            ...
            <div className="relative aspect-square overflow-hidden bg-linear-to-br from-gray-50 to-gray-100/60">
                <span className={cn("absolute left-3 top-3 z-10 rounded-full px-2.5 py-1 text-[14.5px] font-bold", badge.cls)}>
                    {badge.label}
                </span>
                {soldOut && (
                    <span className="absolute right-3 top-3 z-10 rounded-full bg-red-500/90 px-2.5 py-1 text-[14.5px] font-bold text-white">
                        สินค้าหมด
                    </span>
                )}
                <Image
