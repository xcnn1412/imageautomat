...
import { AUTOREEL_VIDEOS, AutoReelVideo } from "@/data/autoreel-videos"
...
export function SignatureReel() {
  ...
  const currentVideo = AUTOREEL_VIDEOS[currentIndex] ?? AUTOREEL_VIDEOS[0]
  ...
  const goNext = useCallback(() => {
    const nextIndex = (currentIndex + 1) % AUTOREEL_VIDEOS.length
    goToSlide(nextIndex)
  }, [currentIndex, goToSlide])

  const goPrev = useCallback(() => {
    const prevIndex = (currentIndex - 1 + AUTOREEL_VIDEOS.length) % AUTOREEL_VIDEOS.length
    goToSlide(prevIndex)
  }, [currentIndex, goToSlide])
      ...
      >
        ...
        <motion.div className="text-center mb-12 lg:mb-16" variants={itemVariants}>
          ...
          <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-deep-space-blue tracking-tight leading-[1.1] mb-4">
            ...
            <br />
            ตู้ REEL ลายเซ็นต์ พร้อมถ่ายภาพ
          </h2>
        ...
        >
          {AUTOREEL_VIDEOS.map((video, index) => (
            <motion.button
              type="button"
              key={video.id}
